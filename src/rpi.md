...
I recommend using an **Ubuntu** image made specifically for your Pi instead of a manufaturer's custom OS like Raspberry Pi OS or Orange Pi OS. Those are designed for simplicity and ease of use at the expense of functionality that will be important to us when building our homelab as a private cloud.
...
Official images for Orange Pi 3B:  
[http://www.orangepi.org](http://www.orangepi.org/html/hardWare/computerAndMicrocontrollers/service-and-support/Orange-Pi-3B.html)  

Official Ubuntu images for all Raspberry Pis:  
[https://ubuntu.com](https://ubuntu.com/download/raspberry-pi)
...
We'll flash the SD card with our `cloud-init` file and other configurations. This will set up `avahi-daemon` to be able to reach the Pi using its hostname rather than having to use its IP. It will also install MicroK8s.  

Replace values for your own environment (your [OS tools to copy/paste](https://ostechnix.com/how-to-use-pbcopy-and-pbpaste-commands-on-linux/), name of your [SSH key](https://www.ssh.com/academy/ssh/keygen), volume name, etc.). This example uses the macOS version of `sed` with an empty string for the mandatory extension parameter when editing a file in-place.  
